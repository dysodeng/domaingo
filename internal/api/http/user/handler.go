package user

import (
	"github.com/CXeon/domaingo/api/http/response"
	appuser "github.com/CXeon/domaingo/internal/application/user"
	tilecontext "github.com/CXeon/tiles/context"
	tileerrors "github.com/CXeon/tiles/errors"
	"github.com/gin-gonic/gin"
)

type Handler struct {
	svc appuser.Service
}

func NewHandler(svc appuser.Service) *Handler {
	return &Handler{svc: svc}
}

func (h *Handler) Register(g *gin.RouterGroup) {
	users := g.Group("/users")
	users.GET("/:uid", h.GetUser)
	users.POST("", h.CreateUser)
	users.PUT("/:uid", h.UpdateUser)
	users.DELETE("/:uid", h.DeleteUser)
}

func (h *Handler) GetUser(c *gin.Context) {
	appCtx := tilecontext.From(c.Request.Context())

	uid := c.Param("uid")
	user, err := h.svc.GetUser(appCtx, uid)
	if err != nil {
		response.Fail(c, err)
		return
	}
	response.OK(c, &UserResponse{
		UID:       user.UID,
		Name:      user.Name,
		Gender:    user.Gender,
		Role:      user.Role,
		Email:     user.Email,
		CreatedAt: user.CreatedAt,
	})
}

func (h *Handler) CreateUser(c *gin.Context) {
	appCtx := tilecontext.From(c.Request.Context())
	var req CreateUserRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Fail(c, tileerrors.ErrBadRequest.Wrap(err))
		return
	}
	uid, err := h.svc.CreateUser(appCtx, &appuser.CreateUserDTO{
		Name:     req.Name,
		Gender:   req.Gender,
		Role:     req.Role,
		Email:    req.Email,
		Password: req.Password,
	})
	if err != nil {
		response.Fail(c, err)
		return
	}
	response.OK(c, &CreateUserResponse{UID: uid})
}

func (h *Handler) UpdateUser(c *gin.Context) {
	appCtx := tilecontext.From(c.Request.Context())
	uid := c.Param("uid")
	var req UpdateUserRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Fail(c, tileerrors.ErrBadRequest.Wrap(err))
		return
	}
	if err := h.svc.UpdateUser(appCtx, &appuser.UpdateUserDTO{
		UID:      uid,
		Name:     req.Name,
		Gender:   req.Gender,
		Role:     req.Role,
		Email:    req.Email,
		Password: req.Password,
	}); err != nil {
		response.Fail(c, err)
		return
	}
	response.OK(c, nil)
}

func (h *Handler) DeleteUser(c *gin.Context) {
	appCtx := tilecontext.From(c.Request.Context())
	uid := c.Param("uid")
	if err := h.svc.DeleteUser(appCtx, uid); err != nil {
		response.Fail(c, err)
		return
	}
	response.OK(c, nil)
}
